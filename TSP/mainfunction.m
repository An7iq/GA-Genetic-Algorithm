...
%算子参数
%选择算子
para.sel=3;% 选择算子 1轮盘赌  2  二元竞标赛制 3  四元竞标赛制
%交叉算子
para.pcr=[1 0.3
    2 0.9
    3 0.5];% 交叉算子 1为单点交叉 2 为双点交叉（交叉中间基因）3 为选取随机的位置 对应各自概率
% 变异算子-顺序
para.pmtseq=[3 1];%  变异若为顺序 1 为两点互换   2 选取1点进行前后互换  3 选取 n个位置 随机排序  对应各自概率
% 变异算子-数值-变化位置
para.pmtnum=[1 0.2
    2 0.3
...
% 每层基因数
Encode.dnum=data.city;%一维数组输入     -- 每段编码的基因位 
%每层取值范围
Encode.bounds{1}=[1:data.city]; % 胞体输入    --0 1.数值要求取值范围二维矩阵 2 数值要求一维矩阵
...
figure
plot(data.coord(result.road,1),data.coord(result.road,2))